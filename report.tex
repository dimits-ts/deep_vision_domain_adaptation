...
\begin{document}
	...
	We use the ResNet-18 \cite{resnet} model for all experiments trained with the Adam optimizer ($lr=0.0005$) and using label smoothing ($\alpha = 0.15$). For the MNIST/MNIST-M dataset in particular, we use the pytorch AdamW implementation with $weight_decay = 10^{-3}$. We set the adaptive threshold rate $\rho = 3$ as in its original paper \cite{ican}. Each ResNet model is pretrained on the ImageNet \cite{imagenet} dataset, before being finetuned on our respective source datasets. Datasets for both domains are randomly split in a 70\%-15\%-15\% training-validation-test split.
